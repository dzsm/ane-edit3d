...
int main ()
  ...
  try
    ...
    std::cout << "." << mm["eee"] << ".";
      ...
      double value = p.Evaluate ("if ( ((1 < yi) && (yi < 8)), 3.9+0.2*rand(100)/100, 4)");

      // display result
      std::cout << "Result = " << value << std::endl;

       value = p.Evaluate ("if ( 1 < yi && yi < 8, 3.9+0.2*rand(100)/100, 4)");

      // display result
      std::cout << "Result = " << value << std::endl;     // example of retrieving a symbol
      double abc = p ["abc"];
